"""
DQN z maskowaniem Q-values przy wyborze akcji.
"""
import os
import random
import numpy as np
...
class DQNCompatibleWrapper(gym.Wrapper):
...
class MaskedDQN(DQN):
    def predict(self, observation, state=None, episode_start=None, deterministic=False):
        ...
        mask = obs_array[:, -NUM_ACTIONS:].astype(bool)

        with torch.no_grad():
            obs_tensor = torch.as_tensor(obs_array, device=self.device).float()
        ...
        if deterministic:
            ...
            if np.random.random() < self.exploration_rate:
                actions = np.array([
                    np.random.choice(np.where(m)[0]) for m in mask
                ])
            else:
                actions = np.argmax(q_values, axis=1)

        return actions if observation.ndim > 1 else int(actions[0]), state
...
if __name__ == "__main__":
    ...
    env = DQNCompatibleWrapper(base_env)
    env = Monitor(env, filename=None)

    os.makedirs(f"logs/{RUN_NAME}", exist_ok=True)
    os.makedirs(f"models/{RUN_NAME}", exist_ok=True)

    print("Tworzenie modelu MaskedDQN...")
    model = MaskedDQN(
        policy="MlpPolicy",
        env=env,
